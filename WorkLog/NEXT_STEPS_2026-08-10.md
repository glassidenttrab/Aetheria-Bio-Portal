...
6. **출시 승인 체크리스트 재산정** — `PRODUCTION_READINESS_AND_SUBSCRIPTION_AUDIT_2026-08-10.md` 10장의 12개 항목을 현재 코드 기준으로 재평가한 결과는 아래와 같습니다.
   ...
   | 8 | 핵심 분석이 실제 데이터/데모임을 오인 불가하게 표시 | 🟡 부분 완료 (2026-08-10 처리, WORK_LOG 23번 항목) | "실시간 표적 선택" 검색이 큐레이션 17개 표적 안에서만 되던 문제를 `api/gene/lookup.ts`로 해결 — UniProt/AlphaFold DB/ChEMBL/Open Targets/PubMed/ClinicalTrials.gov 6개 공개 DB를 실시간으로 조회해 임의의 유전자 검색이 실제로 동작함(`.agents/skills/`의 39개 스킬 문서를 스펙 삼아 이식). 다만 큐레이션 17개 표적의 "AI 표적 스캐너" 심층 리포트(hitCandidates/SMILES/특허 FTO/시장가치)는 여전히 정적 데이터 — 이 부분에 대한 제품 UI 내 고지는 아직 없음 |
   ...
   **요약**: 12개 중 3개 완료(라이브 DB로 확인됨), 5개는 부분 완료, 4개는 미착수. 오늘 진행한 P0~P1 조치, 대표님의 라이브 확인, 실시간 유전자 검색 연동(8번)으로 이 문서 작성 시점(commit `693c62f`) 대비 크게 개선됐지만, 실제 결제를 여는 것은 여전히 이르다고 판단됩니다 — 특히 7(공격 시나리오 테스트)·10(CI)·11(장애 대응)·12(개인정보 절차)는 손도 대지 않은 상태입니다.
